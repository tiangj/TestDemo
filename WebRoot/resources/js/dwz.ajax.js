 ...
 * 
 */
//add by mengxin 20160812 
//弹出dialog进行删除、恢复、审核、驳回、复检时调用刷新当前dialog
/*	
 * 方式一：	
  		<ul class="toolBar">
			<li><a class="delete" target="ajaxTodo" href="${controllerName}/remove?id={sid_list2}" callback="reloadDialog" title="确实要将记录删除至回收站吗?"><span>放入回收站</span></a></li>
			<li><a class="edit" target="ajaxTodo" href="${controllerName}/recovery?id={sid_list2}" callback="reloadDialog"><span>恢复</span></a></li>
		</ul>
*/
/*
 * 方式二：
	<c:choose>
		<c:when test="${item.deleteFlag}">
			<a class="edit" target="ajaxTodo" href="${controllerName}/recovery?id=${item.id}" callback="reloadDialog"><span>恢复</span></a>
		</c:when>
		<c:otherwise>
			<a class="delete" target="ajaxTodo" href="${controllerName}/remove?id=${item.id}" callback="reloadDialog" title="确实要将这些记录删除至回收站吗?"><span>放入回收站</span></a>
		</c:otherwise>
	</c:choose>
*/
